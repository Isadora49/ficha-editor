...
const labels = [
    ...
    "C48 (Imagem 1)", "C49 (Imagem 2)" // Adicionados conforme solicitado
];
...
canvas.addEventListener('click', (e) => {
    ...
    // Ajuste de tamanho visual para campos de imagem
...
btnDownload.addEventListener('click', async () => {
    try {
        ...
        for (let i = 0; i < TOTAL_FIELDS; i++) {
            ...
            // Lógica para criar o tipo correto de campo
            if (i >= 47) {
                // CAMPOS DE IMAGEM (BOTÕES)
                f = form.createButton(name);
                // Não usamos setLabel diretamente para evitar o erro. Usamos a propriedade de botão:
                try { f.setLabel("FOTO"); } catch(e) {}
            } else if (i === 0) {
            ...
            f.addToPage(page, { 
                ...
                borderWidth: i >= 47 ? 1 : 0 
            });

            // Adiciona o script de upload de imagem nos botões C48 e C49
            if (i >= 47) {
                const imgJS = 'event.target.buttonImportIcon();';
                const action = pdfDoc.context.obj({
                    Type: 'Action', S: 'JavaScript', JS: PDFString.of(imgJS)
                });
        ...
        // MOTOR DE CÁLCULO (Preservado conforme seu código)
        const scriptMotor = [
        ...
        triggerNames.forEach(name => {
            try {
                ...
                field.acroField.dict.set(PDFName.of('AA'), pdfDoc.context.obj({ K: calcAction, V: calcAction, Bl: calcAction }));
            } catch(e) {}
        });
